...
class TestESS(unittest.TestCase):
    ...
    def test_full_permutation_matrix(self):
        ...
        for mode, border, metric in itertools.product(search_modes, borders, metrics):
            with self.subTest(mode=mode, border=border, metric=metric):
                ...
                res = ess.ess(
                    self.samples, self.bounds, n=self.n_new,
                    search_mode=mode,
                ...
                self.assertTrue(np.all(new_pts >= -1e-5), f"Lower bound violation: {np.min(new_pts)}")
                self.assertTrue(np.all(new_pts <= 1.00001), f"Upper bound violation: {np.max(new_pts)}")
                ...
                if border == "repulsive":
                    self.assertGreater(ess_min_dist, 1e-5, f"Points stacked! {mode}-{border}-{metric}")
                    self.assertGreater(ess_min_dist, baseline_min_dist * 1.1, f"ESS failed to beat random baseline in configuration {mode}-{border}-{metric}")
                self.assertGreater(ce_index, 1.05, f"Distribution not dispersed enough (R={ce_index}) for {mode}-{border}-{metric}")
    ...
    def test_backend_parity(self):
        ...
        res_numpy = ess.ess(self.samples, self.bounds, n=20, nn_instance=numpy_nn, seed=42)
        dist_numpy = utils.calculate_min_pairwise_distance(res_numpy[1:])
        
        # Force Faiss
        faiss_nn = nn.FaissHNSWFlatNN(dimension=2, seed=42)
        res_faiss = ess.ess(self.samples, self.bounds, n=20, nn_instance=faiss_nn, seed=42)
        ...
        self.assertLess(delta, dist_numpy * 0.15, "Backends diverged significantly in quality")
