...
def _empty_center(coor, data, neigh, *, lr:int, epochs:int, bounds:np.ndarray):
    ...
    for i in range(epochs):
        ...
        adjs_, distances_ = neigh.query_external(coor, k=data.shape[1] + 1)
...
def _esa_01(samples, bounds, n: int = None, seed: int = None):
    ...
    neigh = nn.NumpyNN(dimension=scaled_samples.shape[1], seed=seed)
    neigh.add_static(scaled_samples)

    rng = np.random.default_rng(seed)
    coors = rng.uniform(0, 1, (n, scaled_samples.shape[1]))
    logger.debug(f'Coors({n}, {scaled_samples.shape[1]})\n{coors}')
    ...
    for c in coors:
        ...
        es_param = _empty_center(c.reshape(1, -1), scaled_samples, neigh,
                                 lr=0.01, epochs=100, bounds=scaled_bounds)
...
def _esa_02(samples, bounds, *, n:int=None, epochs:int = 100, lr:float = 0.01, seed:int=None):
    ...
    neigh = nn.NumpyNN(dimension=samples.shape[1], seed=seed)
